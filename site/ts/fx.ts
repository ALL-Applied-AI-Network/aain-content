/**
 * fx.ts — the site's three visual effects, all opt-in via data attributes, all off for
 * prefers-reduced-motion, and all cheap enough to leave on for a marketing page:
 *
 *   <canvas data-aurora>            a slow WebGL flow field in the brand palette (section backdrops)
 *   <img data-liquid src="…">       the image is re-drawn through a displacement shader that answers the cursor
 *   <article data-tilt>             a restrained 3D tilt + glare on hover (showcase cards)
 *
 * Nothing here touches the hero mark; it is decoration around real content, never a replacement for it.
 */
...
/** Runs `draw(t)` only while `el` is on screen and the tab is visible. */
function loopWhileVisible(el: Element, draw: (t: number) => void, fps = 30) {
  ...
  if ("IntersectionObserver" in window) new IntersectionObserver(es => es.forEach(e => (e.isIntersecting ? start() : stop())), { rootMargin: "80px" }).observe(el);
  else start();
...
/* ────────────────────────────── 1 · aurora ────────────────────────────── */
const AURORA_FRAG = `precision mediump float; varying vec2 v; uniform float t; uniform vec2 res; uniform vec2 mouse; uniform float strength;
${NOISE}
void main(){
  vec2 uv = v; uv.x *= res.x / res.y;
  ...
  float vign = smoothstep(0.0, 0.35, v.y) * smoothstep(1.0, 0.55, v.y);
  gl_FragColor = vec4(col * band * vign * strength, 1.0);
}`;
...
export function mountAurora(root: ParentNode = document) {
  root.querySelectorAll<HTMLCanvasElement>("canvas[data-aurora]").forEach(c => {
    ...
    if (REDUCED) { c.style.background = "radial-gradient(60% 60% at 50% 40%, rgba(34,211,238,.18), rgba(168,85,247,.10) 45%, transparent 75%)"; return; }
    ...
    const uT = gl.getUniformLocation(prog, "t"), uR = gl.getUniformLocation(prog, "res"), uM = gl.getUniformLocation(prog, "mouse"), uS = gl.getUniformLocation(prog, "strength");
    ...
    loopWhileVisible(c, t => {
      if (fitCanvas(c, 1)) gl.viewport(0, 0, c.width, c.height);
      mx += (tx - mx) * 0.04; my += (ty - my) * 0.04;
      gl.uniform1f(uT, t); gl.uniform2f(uR, c.width, c.height); gl.uniform2f(uM, mx, my); gl.uniform1f(uS, strength);
...
export function mountLiquid(root: ParentNode = document) {
  if (REDUCED || TOUCH) return;                      // the plain <img> stays; nothing to do
...
export function mountFx(root: ParentNode = document) { mountAurora(root); mountLiquid(root); mountTilt(root); }
