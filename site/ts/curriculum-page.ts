...
function render() {
  ...
  root.innerHTML = '<div class="learn-heading"><div><span class="learn-eyebrow">ALL / LEARN</span><h1>Your Learning <span class="text-grad">Tree</span></h1><p>Explore a branch. Open a chapter to see the lessons and how they connect.</p></div><a class="learn-account" href="https://dashboard.all-ai-network.org/me/learn">Save Your Progress ↗</a></div>'
    + '<div class="learn-toolbar"><button id="all-chapters" ' + (!topic ? 'disabled' : '') + '>← All Chapters</button><label>Find a lesson<input id="lesson-search" type="search" placeholder="Python, chatbots, retrieval…" value="' + esc(query) + '"></label></div>'
    + '<div class="learning-graph__legend"><b>' + esc(group?.title ?? "All Curriculum Chapters") + '</b><span>' + (group ? 'Follow the lesson branches. Open a lesson for its full prerequisites.' : 'Follow the main path. Open a chapter for its lessons.') + '</span></div>'
    + '<div id="search-results" aria-live="polite"></div><div id="chapter-graph" class="learning-graph" role="region" tabindex="0" aria-label="Learning tree: chapters and lesson paths"></div>'
